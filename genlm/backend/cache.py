...
class TokenKVTrie:
    ...
    def __init__(self, parent=None, logprobs=None, key_values=None):
        self.children = {}  # maps token ID to child
        self.logprobs = logprobs  # for next token
    ...
    def __repr__(self):
        return (
            f"{'*' if self.key_values is not None else ''}["
            + ", ".join(
                [
                    f"{node_id}: {node.__repr__()}"
                    for (node_id, node) in self.children.items()
    ...
    def add_token(self, token_id, logprobs=None, key_values=None):
        self.children[token_id] = TokenKVTrie(
            parent=self, logprobs=logprobs, key_values=key_values
        )
        return self.children[token_id]

    def extend_cache(self, next_token_index, token_ids, logits, base, key_values=None):
        node = self
        ...
        for j in range(next_token_index, len(token_ids)):
            ...
            node = node.add_token(token_id, token_logprobs.cpu(), key_values)

        return node

    def extend_cache_from_base(self, token_ids, logits, base, key_values=None):
        node = self

        for j in range(base, len(token_ids)):
            token_id = token_ids[j]
            token_logits = logits[j - base]
            token_logprobs = torch.log_softmax(token_logits, 0)
            kv = None if key_values is None else key_values[j - base]

            if node.has_token(token_id):
                child = node.get_token(token_id)
                # Update existing node if needed
                if child.key_values is None:
                    child.key_values = kv
                if child.logprobs is None:
                    child.logprobs = token_logprobs
            else:
                # Add brand new node
                child = node.add_token(token_id, token_logprobs, kv)
