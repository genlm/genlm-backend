...
parser.add_argument(
    "--batch-sizes", type=str, default="1,4,8,16,32", help="Batch sizes to test"
)
...
def run_benchmark(
    ...
    if use_v0:
        ...
        class V0Wrapper:
            ...
            def batch_next_token_logprobs_sync(self, token_ids_list):
                ...
                prompts = [
                    TokensPrompt(prompt_token_ids=list(t)) for t in token_ids_list
                ]
    ...
    for batch_size in batch_sizes:
        ...
        # Create test prompts
        prompts = [
            llm.tokenizer.encode(f"Test prompt number {i}") for i in range(batch_size)
        ]
        ...
        for i in range(iterations):
            ...
            if batch_size == 1:
                assert result.shape == (
                    llm.vocab_size
                    if hasattr(llm, "vocab_size")
                    else len(llm.tokenizer),
                )
            else:
                expected_vocab = (
                    llm.vocab_size if hasattr(llm, "vocab_size") else len(llm.tokenizer)
                )
        ...
        print(
            f"  Avg latency: {result.avg_latency_ms:.2f}ms ± {result.std_latency_ms:.2f}ms"
        )
...
def print_comparison(
    v0_results: list[BenchmarkResult], v1_results: list[BenchmarkResult]
):
    ...
    for batch_size in sorted(set(v0_by_batch.keys()) | set(v1_by_batch.keys())):
        ...
        if v0 and v1:
            ...
            print(
                f"{batch_size:<8} {v0.avg_latency_ms:>6.2f}ms      {v1.avg_latency_ms:>6.2f}ms      {speedup:>5.2f}x     {v0.throughput_rps:>8.1f} req/s   {v1.throughput_rps:>8.1f} req/s"
            )
        elif v0:
            print(
                f"{batch_size:<8} {v0.avg_latency_ms:>6.2f}ms      {'N/A':<12} {'N/A':<10} {v0.throughput_rps:>8.1f} req/s   {'N/A':<15}"
            )
        elif v1:
            print(
                f"{batch_size:<8} {'N/A':<12} {v1.avg_latency_ms:>6.2f}ms      {'N/A':<10} {'N/A':<15} {v1.throughput_rps:>8.1f} req/s"
            )
...
def main():
    ...
    for r in all_results:
        print(
            f"{r.batch_size:<8} {r.avg_latency_ms:>6.2f}ms      {r.per_request_ms:>6.2f}ms       {r.throughput_rps:>8.1f} req/s"
        )
