...
mass_sum using pytest-benchmark. Compares CPU and GPU parallel implementations against sequential baseline.

pytest benchmark_mass_sum.py --benchmark-only 
"""
...
def test_batch_mass_sum(benchmark, backend, batch_size):
    benchmark.group = f"batch_size={batch_size}"
    trie = load_trie(mock_llm, backend)
    p_llms = torch.exp(torch.stack([
        mock_llm.next_token_logprobs_sync([i])
        for i in range(batch_size)
    ]))
    benchmark(trie.batch_mass_sum, p_llms)
...
class sync_impl:
    ...
    def __call__(self, p_llms):
        return self.trie.batch_mass_sum(p_llms)
...
class async_impl:
    ...
    def __call__(self, p_llms):
        return self.loop.run_until_complete(asyncio.gather(
            *[self.async_trie.mass_sum(p_llm) for p_llm in p_llms]
        ))
...
def test_async_mass_sum(benchmark, impl, backend='parallel-gpu', batch_size=32):
    trie = load_trie(mock_llm, backend)
